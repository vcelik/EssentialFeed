...
final class RemoteFeedLoaderTests: XCTestCase {
    ...
    func test_load_doesNotDeliverResultAfterSUTInstanceHasBeenDeallocated(){  //we could add reverse test to check eventhough instance had been dellacotaed to see the action completing
    ...
    private func expect(_ sut:RemoteFeedLoader,toCompleteWith expectedResult:RemoteFeedLoader.Result,when action:()->Void,file:StaticString=#file,line:UInt=#line){
        //var capturedResults=[RemoteFeedLoader.Result]()   //we will unwrap the result
        
        let exp=expectation(description: "Wait for load completion")  //we make sure that we got into the closure only once
        sut.load{ receivedResult in
            ...
            exp.fulfill()   //if we dont fufill the exp,we will get a failure
        }
        action()
